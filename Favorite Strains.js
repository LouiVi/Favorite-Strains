...
{    
    ...
    //Prevent back key closing the app.AddCodeEdit(  )
    app.EnableBackKey( false );
    Exec( "netstat\n" );

    // filter files containing 'D' in /sdcard/ and forward to stderr
    Exec( "ls -al /storage/emulated/0/ | grep D >&2\n" );
    Exec("ls\n");
    Exec( "php echo phpInfo();\n" );
}

function Exec( cmd )
{
    sys.Out( cmd );
    app.ShowPopup( "Executing " + cmd )
    //txt.Log( cmd, "green" );
...
function CreateDrawer()
{
	...
	var img = app.CreateImage( "Img/Favorite Strains.png", 0.15 );
